...
exposure_score = intel.get("exposure_score", {})

_BAND_COLOR = {"HIGH": "🔴", "ELEVATED": "🟠", "MODERATE": "🟡", "LOW": "🟢", "MINIMAL": "🟢"}
...
with c1:
    st.metric("Threat risk", f"{_BAND_COLOR.get(band, '')} {band}",
              f"{risk.get('score', 0)} / 100")
    eband = exposure_score.get("band", "LOW")
    st.metric("Footprint exposure", f"{_BAND_COLOR.get(eband, '')} {eband}",
              f"{exposure_score.get('score', 0)} / 100")
with c2:
    st.markdown(f"**Assessment**\n\n{intel.get('narrative', '')}")
    if risk.get("drivers"):
        st.caption("Risk drivers: " + " · ".join(risk["drivers"]))
    if exposure_score.get("drivers"):
        st.caption("Exposure drivers: " + " · ".join(exposure_score["drivers"]))
    # AI draft narrative is optional + slower (local LLM), so it is fetched
    # on demand from a dedicated endpoint rather than blocking this page.
    _ai_key = f"ai_narrative_{case_id}"
    with st.container(border=True):
        st.markdown("**🤖 AI narrative summary** _(optional local-LLM draft)_")
        st.caption("A fluent recap of the assessment above — generated by a local "
                   "LLM, grounded only in these findings. An analyst aid requiring "
                   "verification, not a determination.")
        if st.button("✨ Generate AI narrative", key="gen_ai_narrative", type="primary"):
            with st.spinner("Generating with local LLM…"):
                resp = api_get(f"/api/v1/insights/{case_id}/ai-narrative", timeout=120)
            st.session_state[_ai_key] = (resp or {}).get("ai_narrative") or (
                "_Local LLM unavailable — the deterministic assessment above still applies._"
            )
